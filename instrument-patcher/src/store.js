...
const nodeConfig = {
    oscillator: {
        ...
        constructor: (context, value) => new CustomOscillatorNode(context, { frequency: value.data.frequency }),
    },
    filter: {
        defaults: { frequency: 100, Q: 1, label: "filter" },
        constructor: (context, value) => new CustomFilterNode(context, { frequency: value.data.frequency }),
    },
    gain: {
        defaults: { gain: 1, label: "gain" },
        constructor: (context, value) => new CustomGainNode(context, { gain: value.data.gain }),
    },
    delay: {
        defaults: { delay: 0.5, label: "delay" },
        constructor: (context, value) => new EchoNode(context, {
    ...
    sampleandhold: {
        ...
        constructor: (context, value) => new SampleHoldNode(context, { value: value.data.value }),
    },
    slewrate: {
        defaults: { label: "slewrate", value: 0.01 },
        constructor: (context, value) => new SlewRateNode(context, { value: value.data.value }),
    },
    midiCC: {
        defaults: { label: "midiCC", cc: 1, value: 0 },
        constructor: (context, value) => new MidiControlChangeNode(context, {
            cc: value.data.cc,
    ...
    pulse: {
        ...
        constructor: (context, value) => new PulseNode(context, value.data.frequency, value.data.pulseWidth),
    },
    event: {
        defaults: { eventName: 'defaultEvent', interval: 50, label: 'event' },
        constructor: (context, value) => new EventReceiverNode(context, { eventName: value.data.eventName, interval: value.data.interval }),
    },
};
        ...
        if (Array.isArray(data) && data.length > 0) {
            ...
            if (connections && connections[id]) {
                ...
                if (patcher[sourceId] && patcher[targetId]) {
                    ...
                    console.error("Source or target node not found in patcher", sourceId, targetId);
                }
